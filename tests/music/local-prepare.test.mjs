...
test("local preparation enables the real catalog against private media without changing publication content", async () => {
  ...
  try {
    ...
    assert.deepEqual(local.music.items.flatMap((album) => album.tracks).map((track) => track.playback), tracks.map(() => ({ kind: "file", durationMs: record.durationMs })));
    ...
    await writeFile(join(media, "retained-recording.m4a"), "retained audio");
    ...
    assert.equal(await readFile(join(media, "retained-recording.m4a"), "utf8"), "retained audio");
