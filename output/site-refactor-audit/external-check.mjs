...
const data=JSON.parse(await readFile('output/site-refactor-audit/audit.json'));const urls=data.destinations.filter(x=>x.error||x.status>=400).map(x=>x.url);const b=await chromium.launch();let cursor=0;const results=[];
...
await b.close();await writeFile('output/site-refactor-audit/external-browser.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results));
