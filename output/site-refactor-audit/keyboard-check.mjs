...
const r=JSON.parse(await readFile('output/site-refactor-audit/links.partial.json'));const candidates=r.links.filter(x=>x.result==='click-failed'&&x.text.startsWith('Skip'));
...
await b.close();await writeFile('output/site-refactor-audit/keyboard.json',JSON.stringify(results,null,2));console.log(JSON.stringify({checked:results.length,missing:results.filter(x=>!x.fragmentExists)}));
